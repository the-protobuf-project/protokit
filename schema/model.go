...
type Table struct {
	...
	// Source is the proto message descriptor this table was built from — a generic
	// provenance handle a generator's enrichment pass uses to read its own
	// message-level options (e.g. web3.v1.table) without protokit knowing those
	// options exist. Nil for synthesized tables (many-to-many join tables) that
	// map to no message.
	...
	// SchemaVersion is a deterministic fingerprint of the table's final column
	// layout (a 0x-prefixed 32-byte hash), set by the web3 enrichment pass after
	// relations resolve. The solidity contract exposes it as SCHEMA_VERSION so the
	// evm driver can refuse a contract whose storage layout has drifted from the
	// generated client — the immutable-storage counterpart of a migration check.
...
type Column struct {
	...
	// Type is the neutral, target-agnostic type classified from the proto field.
	// Each generator maps it to its own type system. Enum fields set Type=TypeEnum
	// and Column.Enum.
	Type FieldType

	// List is true when the proto field is repeated (cardinality), kept separate
	// from Type so a repeated field keeps its element FieldType.
	List bool

	// Enum points to the enum definition when the proto field is an enum kind.
	// Backends render their native enum reference instead of a projected Type.
	Enum *Enum
	...
	// Unique is true when the orm enrichment pass reads orm.v1.column.unique = true.
	Unique bool

	// Default is a backend default expression. protokit sets it for a required enum
	// column (the first declared value, an AIP-126 convention); the orm enrichment
	// pass overrides it from orm.v1.column.default_value.
	Default string

	// Index is true when the orm enrichment pass reads orm.v1.column.index = true
	// (single-column index).
	Index bool
	...
	// Source is the proto field descriptor this column was built from — a generic
	// provenance handle a generator's enrichment pass uses to read its own
	// field-level options (e.g. orm.v1.column) without protokit knowing those
...
// Index describes a multi-column index. protokit synthesizes an index for a
// foreign-key column; the orm enrichment pass adds those declared via
// orm.v1.table.indexes.
type Index struct {
