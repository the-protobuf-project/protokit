...
// colName returns the backend's column-name override if set, otherwise the proto
// field name (already snake_case).
func colName(f *protogen.Field, cs schema.ColumnStructure) string {
	if cs.Column != "" {
		return cs.Column
	}
...
// AIP annotation accessors. The generic structural options (grouping, names,
// key/timestamp synthesis, FK behavior) are read from the generator's own package
// through the schema.Backend, not here — protokit imports no generator's options.
// Each accessor returns a safe default when its extension is absent.
