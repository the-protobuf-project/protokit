...
// finalizeIndexes runs the index passes that must see the complete index set —
// including any a generator's enrichment added (e.g. orm.v1.table.indexes). It
// synthesizes an index for every foreign-key column, auto-names unnamed indexes
// (idx_<table>_<cols>), and validates their columns exist. Kept out of
// buildDatabases so it runs after enrichment: a declared index must be present
// before it is named and before the FK indexes are appended, so both the order
// and the generated names match a single-pass build.
func finalizeIndexes(dbs []*schema.Database, diags *diagnostics) {
	for _, db := range dbs {
		indexForeignKeys(db)
		nameIndexes(db)
...
func (ctx *buildCtx) mergeFile(byName map[string]*schema.Database, f *protogen.File) (*schema.Database, error) {
	// The backend resolves the datasource: database/schema from its own annotation
	...
	provider, err := types.ParseProvider(ds.Provider)
	if err != nil {
	...
	db, ok := byName[name]
	if !ok {
		db = &schema.Database{Name: name, URL: ds.URL, Provider: string(provider)}
		byName[name] = db
	} else {
		if db.URL == "" {
			db.URL = ds.URL
		}
		if db.Provider != string(provider) && ds.Provider != "" {
			return nil, fmt.Errorf("%s: datasource %q provider %q conflicts with %q",
				f.Desc.Path(), name, provider, db.Provider)
		}
	}

	if added := ctx.addFileTables(db, f, ds.Schema, ds.SchemaStrip); !added && !ok {
		delete(byName, name)
		return nil, nil
	}
	return db, nil
...
func (ctx *buildCtx) addFileTables(db *schema.Database, f *protogen.File, schemaOverride string, stripVer bool) bool {
	...
	for _, msg := range f.Messages {
		...
		sName, tName := schemaTable(res.GetType(), ts.Table)
		// google.api.resource.plural is the authoritative plural ("shelves"),
		// beating the naive +s inference. An explicit table-name override still wins.
		if ts.Table == "" && res.GetPlural() != "" {
